...
def index():
    ...
    Msg = greeting() + session["username"].upper() + ". What would you like to do today?"
...
def buy():
    ...
    if request.method == "POST":
        ...
        if cost < check:
            ...
            flash("You have successfully purchased " + str(share_count) + " of " +
                  symbol.upper() + " at " + str(stock_info["price"]) + " per share.")
            return redirect("/")
        else:
...
def sell():
    ...
    if request.method == "POST":
        ...
        if found == 1:
            ...
            flash("You have successfully sold " + str(share_count) + " share of " +
                  stock_data["symbol"].upper() + " at " + str(stock_data["price"]) + " per share.")
            return redirect('/')
        else:
